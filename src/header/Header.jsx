...
const Header = () => {
  const navigate = useNavigate();
    ...
    const toggleSidebar = () => {
        if (isSidebarOpen) {
          console.log('isCollapsed', isCollapsed, !isCollapsed);
          setIsCollapsed(!isCollapsed); // Toggle between full and collapsed states
        } else {
          setIsSidebarOpen(true);
          console.log('002', isSidebarOpen);
        }
      };
  ...
  return (
    <div 
    className={`${
        isSidebarOpen ? (isCollapsed ? "w-full ml-20" : "w-64  ml-60") : "w-0"
      } fixed t-0 w-full px-5 py-5 bg-red-500 transition-all duration-300 z-40 overflow-hidden`}
    ...
    >
        ...
        {/* Sidebar */}
            <aside
        className={`${
          isSidebarOpen ? (isCollapsed ? "w-20" : "w-64") : "w-0"
